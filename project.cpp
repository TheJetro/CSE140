...
string ControlUnit(string operation){
    ...
    if (toInt(operation) == 35) {
        ...
        //if operation is SW
        //RegDST = X, MemtoReg = X should be x but zero as a stand-in
        RegDst = 0, ALUSrc = 1, MemtoReg = 0, RegWrite = 0, MemRead = 0, MemWrite = 1, Branch = 0;
        Jump = 0, InstType = 0;
        alu_op = "0010";
    }else if (toInt(operation) == 32){
        //if operation is ADD 
